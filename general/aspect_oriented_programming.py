...
class Aspecter(type):
    ...
    def matching_pre_functions(cls, method, args, kw):
        ...
        return [
            ...
            and (
                rule["in_objects"] == ()
                or any((type(arg) in rule["in_objects"] for arg in all_args))
            )
        ]
    ...
    def matching_post_functions(cls, method, results):
        ...
        return [
            ...
            and (
                rule["out_objects"] == ()
                or any((type(result) in rule["out_objects"] for result in results))
            )
        ]
...
if __name__ == "__main__":
    ...
    def log_address(*args, **kw):
        addresses = [arg for arg in (args + tuple(kw.values())) if type(arg) == Address]
        print(addresses)
