...
for i in range(
    numero_casos
):  # Se ingresan los datos de las variables y se gurdan en una lista
    ...
    instruccion = str(
        input(
            f"Inserte las letras L y R (exactamente {pasos} letras sin espacios) para instruir al robot:"
        )
    ).upper()
    while len(instruccion) != pasos:
        ...
        if len(instruccion) > pasos:
            ...
            instruccion = str(
                input(f"Inserte de nuevo las {pasos} letras L y R sin espacios:")
            ).upper()
