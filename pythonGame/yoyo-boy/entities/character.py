...
class Character(pygame.sprite.Sprite):
    ...
    def idle_animation(self, canvas):
        ...
        canvas.blit(self.animation_images['idle'][self.facing_right][self.idle_iter], self.rect)
    ...
    def attack_animation(self, canvas):
        ...
        canvas.blit(self.animation_images['attack'][self.facing_right][self.attack_iter], self.rect)
        self.attack_iter = (self.attack_iter + 1) % ATTACK_SUB_IMAGES
        if self.attack_iter == len(self.animation_images['attack']) - 1: self.is_attack = False

    def run_animation(self, canvas):
        self._load_animation('run', RUN_SUB_IMAGES)
        canvas.blit(self.animation_images['run'][self.facing_right][self.run_iter], self.rect)
        self.run_iter = (self.run_iter + 1) % RUN_SUB_IMAGES
        if self.run_iter == len(self.animation_images['run']) - 1: self.is_running = False

    def attack(self):
        self.is_attack = True
        
    def set_animation(self, canvas):
        if self.is_attack: self.attack_animation(canvas)
        elif self.is_running: self.run_animation(canvas)
        else: self.idle_animation(canvas)
