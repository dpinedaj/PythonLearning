...
def pruebaHeaders():
    ...
    response = requests.post("http://httpbin.org/post", timeout=5, data=datos, headers=headers)
...
def pruebaPut():
    ...
    files = {'file': ('file.xlsx', open('file.xlsx', 'rb'), 'application/vnd.ms-excel',
            {'Expires':'0'})}
    response = requests.put("http://httpbin.org/patch", timeout=5, data=datos, files=files)
