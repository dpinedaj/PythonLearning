...
class ImageInfo:
    ...
    def get_lat_long(self):
        gpsinfo = {}
        if "GPSInfo" in self.exifinfo:
            Nsec = self.exifinfo["GPSInfo"][2][2][0] / float(
                self.exifinfo["GPSInfo"][2][2][1]
            )
            Nmin = self.exifinfo["GPSInfo"][2][1][0] / float(
                self.exifinfo["GPSInfo"][2][1][1]
            )
            Ndeg = self.exifinfo["GPSInfo"][2][0][0] / float(
                self.exifinfo["GPSInfo"][2][0][1]
            )
            Wsec = self.exifinfo["GPSInfo"][4][2][0] / float(
                self.exifinfo["GPSInfo"][4][2][1]
            )
            Wmin = self.exifinfo["GPSInfo"][4][1][0] / float(
                self.exifinfo["GPSInfo"][4][1][1]
            )
            Wdeg = self.exifinfo["GPSInfo"][4][0][0] / float(
                self.exifinfo["GPSInfo"][4][0][1]
            )
