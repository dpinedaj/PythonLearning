from quart import (
    Quart, abort, redirect, render_template, request, session,
    url_for, g, flash
)
...
# 1. Define app, set config with environment variables such user, pwd, database
app = Quart(__name__)
...
# 2. Util functions to handle data
def connect_db():
...
# 3. init_db to set the first database with the schema using an sql file
...
# 4. Function to handle posts display, uses the render template
...
# 5 Function to create the new posts, can be used by a template to send a post
