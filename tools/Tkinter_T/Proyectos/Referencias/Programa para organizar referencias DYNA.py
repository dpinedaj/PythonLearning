while True:
    ...
    m = int(
        input(
            "Seleccione lo que vas a ingresar\
    \n1.Revista impresa\
    ...
    if m == 1:
        ...
        lugar = input(
            "Lugar (i.e. Facultad de Minas, Universidad Nacional de Colombia, sede Medellín): "
        )
    ...
    if m == 2:
        ...
        for i in range(n):
            ...
            print(
                f"Inserta el(las) iniciales del nombre del {i+1} autor (i.e. D A, D)",
                end=": ",
            )
            nombre.append(input().replace(" ", "."))

        titulo_articulo = input(
            "Inserta el Título del Artículo (i.e. Efecto de dos índices de...): "
        )
        titulo_revista = input(
            "Inserta el Título de la Revista (i.e. Revista Facultad de Minas): "
        )
