...
def ec_miffin():
    ...
    if str(var.get().strip()) == "Hombre":
      try:
         miffin = (10 * int(peso.get())) + (6.25 * int(estatura.get()) - (5 * int(edad.get()))) + 5
      except:
         miffin = "Ingresa tus datos"
    elif str(var.get().strip()) == "Mujer" :
        try:
            miffin = (10 * int(peso.get())) + (6.25 * int(estatura.get()) - (5 * int(edad.get()))) - 161
        except:
           miffin = "Ingresa tus datos"
...
imb = tk.Button(ventana, text="TMB", command=ec_miffin,
                    bg="light salmon", fg="black")
imb.place(x=300, y=40, height=100, width=100)

#----------------------------------------------------CÁLCULO DEL GASTO CALÓRICO------------------------------------------

e5 = tk.Label(ventana,text = "Ahora vamos a calcular tu gasto calórico",bg = "lavender", font = ("Amaranth", 15), fg = "black")
...
gasto = tk.Button(ventana, text="Gasto", command=gasto,
                    bg="light salmon", fg="black")
